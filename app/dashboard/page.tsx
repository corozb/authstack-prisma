...
export default async function DashboardPage() {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const weekLabel = `${String(weekStart.getMonth() + 1).padStart(2, "0")}/${String(weekStart.getDate() + 1).padStart(
      2,
      "0"
    )}`;
  ...
  const lowStockCount = allProducts.filter((product) => +product.quantity > 5 && Number(product.quantity) >= 1).length;
  ...
  const outOfStockPercentage = totalProducts > 0 ? Math.round((outStockCount / totalProducts) * 100) : 0;
